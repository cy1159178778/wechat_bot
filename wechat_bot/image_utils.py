...
from typing import Awaitable, Callable, List, Literal, Optional, Tuple, Union, Any
...
class BuildImage:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.font = ImageFont.truetype(str(Path(FONT_PATH) / font), int(font_size))
    ...
    def load_font(cls, font: str, font_size: Optional[int]) -> FreeTypeFont:
        ...
        """
        return ImageFont.truetype(str(Path(FONT_PATH) / font), font_size or cls.font_size)
    ...
    def getsize(self, msg: str) -> Tuple[int, int]:
        ...
        参数:
            :param msg: 文字内容
        """
        return self.font.getbbox(msg)[2:4]
    ...
    def resize(self, ratio: float = 0, w: int = 0, h: int = 0):
        ...
        if not w and not h and ratio:
            w = int(self.w * ratio)
            h = int(self.h * ratio)
        self.markImg = self.markImg.resize((w, h), Image.Resampling.LANCZOS)
    ...
    def check_font_size(self, word: str) -> bool:
        ...
        参数:
            :param word: 文本内容
        """
        return self.font.getbbox(word)[2:4][0] > self.w
    ...
    def circle(self):
        ...
        if size[0] != size[1]:
            self.markImg = self.markImg.resize((r2, r2), Image.Resampling.LANCZOS)
        width = 1
        antialias = 4
        ...
        mask = mask.resize(self.markImg.size, Image.Resampling.LANCZOS)
        try:
