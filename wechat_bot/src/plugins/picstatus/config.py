...
DEFAULT_AVATAR_PATH = Path(__file__).parent / "avatar.jpg"
...
class ConfigModel(BaseModel):
    ...
    ps_command: List[str] = ["运行状态", "状态", "zt", "yxzt", "status", "运行状况", "运行时长", "运行信息"]
    ps_only_su: bool = False
    ps_need_at: bool = False
    ps_reply_target: bool = False
    ...
    ps_test_sites: List[TestSiteCfg] = [
        ...
        TestSiteCfg(name="Bing", url="https://cn.bing.com/"),
...
dic = {'superusers': set(), 'nickname': set()}


config: ConfigModel = ConfigModel.parse_obj(dic)
