...
class BaseHandle(Generic[TC]):
    def __init__(self, game_name: str, game_name_cn: str):
        ...
        self.data_path = Path(DATA_PATH) / "draw_card"
        self.img_path = Path(DRAW_PATH) / f"{self.game_name}"
        self.up_path = Path(DATA_PATH) / "draw_card" / "draw_card_up"
    ...
    def init_data(self):
        try:
            ...
            print(f"{self.game_name_cn} 导入角色数据错误：{type(e)}：{e}")
    ...
    async def update_info(self):
        try:
            ...
            print(f"{self.game_name_cn} 更新数据错误：{type(e)}：{e}")
        self.init_data()
    ...
    async def get_url(self, url: str) -> str:
        ...
        for i in range(retry):
            try:
                ...
                print(f"访问 {url} 超时, 重试 {i + 1}/{retry}")
    ...
    async def download_img(self, url: str, name: str) -> bool:
        ...
        try:
            ...
            print(f"下载 {self.game_name_cn} 图片超时，名称：{name}，url：{url}")
            return False
        except:
            print(f"下载 {self.game_name_cn} 链接错误，名称：{name}，url：{url}")
            return False
